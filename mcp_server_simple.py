#!/usr/bin/env python3
"""
Simple MCP Server for Customer JIRA Tracker
"""
...
class CustomerJiraTrackerAPI:
    ...
    async def get_customer_tickets(self, customer_name: str) -> Dict:
        ...
        return await self._make_request("GET", f"/api/customers/{customer_name}/tickets")
    ...
    async def add_customer_tickets(self, customer_name: str, ticket_keys: List[str], notes: Optional[str] = None) -> Dict:
        ...
        if notes:
            data["notes"] = notes
        return await self._make_request("POST", f"/api/customers/{customer_name}/tickets", data)
    ...
    async def add_ticket_comment(self, customer_name: str, ticket_key: str, comment: str) -> Dict:
        ...
        return await self._make_request("POST", f"/api/customers/{customer_name}/tickets/{ticket_key}/comments", data)
    
    async def update_customer_notes(self, customer_name: str, notes: str) -> Dict:
        """Update customer notes"""
        data = {"notes": notes}
        return await self._make_request("PUT", f"/api/customers/{customer_name}/notes", data)
    
    async def remove_customer_tickets(self, customer_name: str, ticket_keys: List[str]) -> Dict:
        """Remove tickets from a customer"""
        return await self._make_request("DELETE", f"/api/customers/{customer_name}/tickets", ticket_keys)
    
    async def list_customers(self) -> Dict:
        """List all customers"""
        return await self._make_request("GET", "/api/customers")
    
    async def export_customer_data(self, customer_name: str, format_type: str = "markdown", include_jira: bool = False, save_file: bool = True) -> Dict:
        """Export customer data in specified format"""
        endpoint = f"/api/customers/{customer_name}/export"
        query_params = f"?format={format_type}&include_jira={str(include_jira).lower()}&save_file={str(save_file).lower()}"
